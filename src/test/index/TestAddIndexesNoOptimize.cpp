...
static IndexWriter4Test * newWriter(Directory::Pointer dir, WhitespaceAnalyzer * analyzer, bool create) {
...
static void verifyNumDocs(CuTest *tc, Directory::Pointer dir, int numDocs) {
...
static void verifyTermDocs(CuTest *tc, Directory::Pointer dir, Term::Pointer term, int numDocs) {
...
void setUpDirs(CuTest *tc, Directory::Pointer dir, Directory::Pointer aux) {
...
void testSimpleCase(CuTest *tc) {
    ...
    Directory::Pointer aux(_CLNEW RAMDirectory());
    Directory::Pointer aux2(_CLNEW RAMDirectory());
    ...
    Directory::Pointer aux3(new RAMDirectory());
    ...
    assertEquals(190, writer->docCount());
    {
        CLVector<Directory::Pointer, Directory::Deletor> dirs(1);
        dirs[0] = aux3;
        writer->addIndexesNoOptimize( dirs );
    }
    ...
    Term::Pointer t1(new Term(_T("content"), _T("aaa")));
    Term::Pointer t2(new Term(_T("content"), _T("bbb")));
    ...
    Directory::Pointer aux4(new RAMDirectory());
...
void testAddSelf(CuTest * tc)  {
    ...
    Directory::Pointer dir(new RAMDirectory());
    // auxiliary directory
    Directory::Pointer aux(new RAMDirectory());
...
void testMoreMerges(CuTest * tc)  {
    ...
    // auxiliary directory
    Directory::Pointer aux(new RAMDirectory());
    Directory::Pointer aux2(new RAMDirectory());
    ...
    CLVector<Directory::Pointer, Directory::Deletor> dirs2;
    dirs2.resize(2);
    dirs2[0] = aux;
    dirs2[1] = aux2;
...
void testHangOnClose(CuTest * tc) {
    ...
    WhitespaceAnalyzer an;
    Directory::Pointer dir(new MockRAMDirectory());
    boost::static_pointer_cast<MockRAMDirectory>(dir)->setRandomIOExceptionRate(0.0, 0);
    ...
    MockRAMDirectory::Pointer dir2(new MockRAMDirectory());
    boost::static_pointer_cast<MockRAMDirectory>(dir2)->setRandomIOExceptionRate(0.0, 0);
