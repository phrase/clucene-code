...
void testBooleanScorer2WithProhibitedScorer(CuTest* tc) {
    ...
    Scorer::AutoPtr prohibitedScorer(new MockScorer(&similarity));
    scorer.add(prohibitedScorer, false, true);
    CL_NS(search)::MockHitCollector collector;
    scorer.score(&collector);

    CuAssertIntEquals(tc, _T("Unexpected calls of next()!"), 1, static_cast<MockScorer*>(prohibitedScorer.get())->getNextCalls());
}
