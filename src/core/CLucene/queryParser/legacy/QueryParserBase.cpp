...
//Post - A query instance has been returned for the specified field
  ...
  if (v.size() == 0){
    ...
    if (v.size() == 1){
      boost::shared_ptr<Term> t(_CLNEW Term(field, v[0]));
      ...
      if (severalTokensAtSamePosition) {
        if (positionCount == 1) {
          ...
          while ( itr != v.end() ){
            boost::shared_ptr<Term> t(_CLNEW Term(field, *itr));
        ...
        while ( itr != v.end() ){
          ...
          boost::shared_ptr<Term> t(_CLNEW Term(field, data));
          q->add(t);
          ++itr;
        }
        return q;
...
//Post - A PrefixQuery instance has been returned
  ...
  boost::shared_ptr<Term> t(_CLNEW Term(field, termStr));
  CND_CONDITION(t.get() != NULL,"Could not allocate memory for term t");
...
CL_NS(search)::Query* QueryParserBase::GetRangeQuery(const TCHAR* field, TCHAR* part1, TCHAR* part2, bool inclusive){
  ...
  boost::shared_ptr<Term> t1(_CLNEW Term(field,part1));
  boost::shared_ptr<Term> t2(_CLNEW Term(field,part2));
