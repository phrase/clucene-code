...
CL_NS(search)::Scorer::AutoPtr SpanWeight::scorer( CL_NS(index)::IndexReader* reader )
{
    return CL_NS(search)::Scorer::AutoPtr(
        _CLNEW SpanScorer( query->getSpans( reader ), 
                              this,
                              similarity,
...
CL_NS(search)::Explanation * SpanWeight::explain( CL_NS(index)::IndexReader* reader, int32_t doc )
{
    ...
    {
        CL_NS(index)::Term::ConstPointer term = (*itTerms);
    ...
    Scorer::AutoPtr pScorer = scorer( reader );
