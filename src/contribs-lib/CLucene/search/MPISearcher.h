...
------------------------------------------------------------------------------*/
#ifndef _lucene_search_mpisearcher
#define _lucene_search_mpisearcher
...
CL_NS_DEF(search)
 ...
 /** Implements MPI-based parallel search over a set of <code>Searchables</code>.
  *
  * Unless documented otherwise, results are only specified for the process with
  * rank 0
  *
  * <p>Applications usually need only call the inherited {@link #search(Query)}
  * or {@link #search(Query,Filter)} methods.
  */
class CLUCENE_CONTRIBS_EXPORT MPISearcher: public Searcher {
private:
...
protected:
    ...
    /** returns true if the active MPI rank processes the sub-searcher that
     *  contains the document. */
    bool selfSubSearcher(int32_t n) const;

    /** converts a Comparable array to its serializable form
     *                                                       */
    std::vector<comparable_value> packed_comparables(
        CL_NS(util)::Comparable** unpacked
    );

    /** converts a Comparable array out of its serializable form
     *                                                       */
    CL_NS(util)::Comparable** unpacked_comparables(
        std::vector<comparable_value> const& packed
    );
public:
    /** Creates a searcher which searches <i>Searchables</i>. */
    MPISearcher(boost::mpi::communicator world, Searchable** searchables);
